...
export default function Interiors() {
  ...
  const lightInt = lightingTheme === 'night' ? 2 : 0;
  const lampInt = lightingTheme === 'night' ? 3 : 0;
  const screenGlow = lightingTheme === 'night' ? 1.5 : 0.4;
  
  const masterBedX = layoutType === '2bhk' ? -width / 4 + 1.5 : 0;

  // Premium Blueprint Specification Labels (Vercel Type-Safe)
  ...
  const ArchitecturalDoor = ({ pos, rotation = [0, 0, 0], swingInvert = false }: { pos: [number, number, number], rotation?: [number, number, number], swingInvert?: boolean }) => {
    ...
    const hingeX = swingInvert ? 1.5 : -1.5;

    return (
      <group position={pos} rotation={rotation}>
        <mesh position={[-1.6, 3, 0]}><boxGeometry args={[0.2, 6, 0.5]} /><meshStandardMaterial color="#1c1917" roughness={0.9} /></mesh>
        <mesh position={[1.6, 3, 0]}><boxGeometry args={[0.2, 6, 0.5]} /><meshStandardMaterial color="#1c1917" roughness={0.9} /></mesh>
        <mesh position={[0, 5.9, 0]}><boxGeometry args={[3.4, 0.2, 0.5]} /><meshStandardMaterial color="#1c1917" roughness={0.9} /></mesh>
        
        <group position={[hingeX, 0, 0]} rotation={[0, swingAngle, 0]}>
          <mesh position={[-hingeX, 2.9, 0]}><boxGeometry args={[3, 5.6, 0.12]} /><meshStandardMaterial color="#292524" roughness={0.6} /></mesh>
          <mesh position={[-hingeX, 2.9, 0.07]}><boxGeometry args={[2.5, 5.2, 0.02]} /><meshStandardMaterial color="#3f3f46" roughness={0.8} /></mesh>
          <mesh position={[-hingeX + (swingInvert ? -1.2 : 1.2), 3, 0.15]}><boxGeometry args={[0.05, 1.5, 0.05]} /><meshStandardMaterial color="#e4e4e7" metalness={1} roughness={0.2} /></mesh>
          <mesh position={[-hingeX + (swingInvert ? -1.2 : 1.2), 3, -0.15]}><boxGeometry args={[0.05, 1.5, 0.05]} /><meshStandardMaterial color="#e4e4e7" metalness={1} roughness={0.2} /></mesh>
        </group>
      </group>
    );
  ...
  const SplitAC = ({ pos, rot }: { pos: [number, number, number], rot: [number, number, number] }) => (
    <group position={pos} rotation={rot}>
      ...
      <mesh position={[1, -0.1, 0.42]}><boxGeometry args={[0.2, 0.1, 0.01]} /><meshStandardMaterial color="#38bdf8" emissive="#38bdf8" emissiveIntensity={lampInt} /></mesh>
    </group>
  );
  ...
  return (
    <group>
      ...
      {layoutType !== 'studio' && (
        <>
          {/* Main Entrance properly placed at the side to not block the view */}
          <ArchitecturalDoor pos={[width * 0.15, 0, w1]} /> 
          {/* Bedroom Doors aligned to side passages */}
          <ArchitecturalDoor pos={[-width * 0.25, 0, w3]} /> 
          {layoutType === '2bhk' && <ArchitecturalDoor pos={[width * 0.25, 0, w3]} swingInvert={true} />}
        </>
      )}
      ...
      <group position={[0, 0, cPark]}>
        ...
        {/* Luxury Compound Sliding Gate */}
        <group position={[0, 0, dPark/2 - 0.2]}>
          <mesh castShadow position={[-width/2 + 0.5, 2.5, 0]}><boxGeometry args={[1, 5, 1]} /><meshStandardMaterial color="#292524" /></mesh>
          <mesh castShadow position={[width/2 - 0.5, 2.5, 0]}><boxGeometry args={[1, 5, 1]} /><meshStandardMaterial color="#292524" /></mesh>
          <group position={[width/4, 2, 0]}>
            <mesh><boxGeometry args={[width/2, 4, 0.2]} /><meshStandardMaterial color="#0f172a" metalness={0.9} roughness={0.2} /></mesh>
            {[-3, -1.5, 0, 1.5, 3].map(x => (
              <mesh key={x} position={[x, 0, 0.15]}><boxGeometry args={[0.1, 4, 0.05]} /><meshStandardMaterial color="#fbbf24" metalness={1} /></mesh>
            ))}
          </group>
        </group>
      ...
      <group position={[0, 0, cLiv]}>
        ...
        <SplitAC pos={[0, 4.5, dLiv/2 - 0.5]} rot={[0, Math.PI, 0]} />
        ...
        <group position={[0, 0, dLiv/2 - 0.3]}>
          ...
          <mesh castShadow position={[0, 1.3, 0.4]}><boxGeometry args={[4, 0.15, 0.2]} /><meshStandardMaterial color="#09090b" metalness={0.8} /></mesh> 
          
          {/* PS5 Style Console */}
          <group position={[3, 1.45, 0.4]}>
        ...
        <group position={[-width/2 + 5, 0, -1]}>
          ...
          <mesh position={[2.8, 0.75, 1]}><boxGeometry args={[0.6, 0.05, 0.4]} /><meshStandardMaterial color="#18181b" /></mesh> 
      ...
      <group position={[0, 0, cKit]}>
        ...
        <group position={[width/2 - 3.2, 0, 0]}>
          ...
          {/* Luxury Espresso Machine (Fixed R3F Error) */}
          <group position={[-1, 1.8, dKit/2 - 1.2]}>
            ...
            <mesh position={[0, 0, -0.26]} rotation={[Math.PI/2, 0, 0]}><cylinderGeometry args={[0.1, 0.1, 0.05]} /><meshStandardMaterial color="#cbd5e1" metalness={1} /></mesh>
          </group>
        ...
        <group position={[-width/2 + 3.2, 0, 0]}>
          ...
          {/* Bathtub (Fixed R3F Error) */}
          <group position={[0.8, 0.6, -dKit/2 + 3.8]}>
            <mesh castShadow><cylinderGeometry args={[0.8, 0.6, 1.2, 32]} /><meshPhysicalMaterial color="#f8fafc" roughness={0.1} clearcoat={1} /></mesh>
            <mesh position={[-1, 0.8, 0]}><cylinderGeometry args={[0.04, 0.04, 1.6]} /><meshStandardMaterial color="#cbd5e1" metalness={1} /></mesh>
            <mesh position={[-0.8, 1.6, 0]} rotation={[0, 0, Math.PI/2]}><cylinderGeometry args={[0.04, 0.04, 0.4]} /><meshStandardMaterial color="#cbd5e1" metalness={1} /></mesh>
          </group>
          ...
          <mesh castShadow position={[1.4, 0.8, dKit/2 - 1.8]} rotation={[Math.PI/2, 0, 0]}><cylinderGeometry args={[0.65, 0.55, 1.2, 32]} /><meshStandardMaterial color="#ffffff" roughness={0.1} /></mesh>
          <mesh castShadow position={[1.4, 1.5, dKit/2 - 1.1]}><boxGeometry args={[1.5, 3, 0.3]} /><meshStandardMaterial color="#e2e8f0" /></mesh>
          <mesh position={[1.4, 2, dKit/2 - 1.26]}><boxGeometry args={[0.4, 0.2, 0.02]} /><meshStandardMaterial color="#94a3b8" metalness={1} /></mesh>
        </group>
      </group>
      ...
      <group position={[0, 0, cBed]}>
        ...
        <group position={[masterBedX, 0, 0]}>
          ...
          <SplitAC pos={[0, 4.5, -dBed/2 + 0.5]} rot={[0, 0, 0]} />
          ...
          <group position={[0, 4.5, -dBed/2 + 0.2]}>
            ...
            <mesh position={[0, 0, 0.06]}><boxGeometry args={[3.8, 1.8, 0.02]} /><meshStandardMaterial color="#fcd34d" metalness={0.6} /></mesh>
          </group>
          ...
          <group position={[1, 0.82, -dBed/2 + 8.8]}>
             <mesh rotation={[0, -0.2, 0]}><boxGeometry args={[0.8, 0.02, 0.6]} /><meshStandardMaterial color="#94a3b8" metalness={1} /></mesh>
             <mesh position={[0, 0.25, -0.3]} rotation={[0.4, -0.2, 0]}><boxGeometry args={[0.8, 0.5, 0.02]} /><meshStandardMaterial color="#020617" /></mesh>
          </group>
        ...
        {[-3.5, 3.5].map((x) => (
           <mesh key={`curtain-${x}`} position={[x, 3, dBed/2 - 0.2]}>
             <boxGeometry args={[1.5, 6, 0.1]} />
